...
class BaseModel 
{
	...
	public static function all(){
	 ...
	 public static function selectJoin($query1,$query2,$query3){
		$model = new static();
		// $model->queryBuilder = "select * from $model->tableName where $arr[0] $arr[1] '$arr[2]'";
		$model ->queryBuilder = "SELECT $query1 FROM $model->tableName INNER JOIN $query2 WHERE $query3";
	   return $model;
	}
	public static function pushAnswer(){
		$model = new static();
		// $model->queryBuilder = "select * from $model->tableName where $arr[0] $arr[1] '$arr[2]'";
		$model ->queryBuilder = "SELECT * FROM $model->tableName   ORDER BY id DESC LIMIT 1 ";
	   return $model;
	}
}
